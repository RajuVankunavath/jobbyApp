...
class JobItemDetails extends Component {
  ...
  getJobItemDetails = async () => {
    ...
    if (response.ok) {
      const newJobDetails = data.job_details
      const jobDetails = {
        ...
        skills: newJobDetails.skills.map(eachValue => ({
      ...
      const similarJobs = similarJob.map(eachValue => ({
  ...
  render() {
    return (
      <>
        <Header />
        <div className="job-item-main-container">{this.getItemView()}</div>
      </>
    )
  }
}
